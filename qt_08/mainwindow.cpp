...
void MainWindow::on_pb_request_clicked()
{
    ...
    if (category == "Все") {
        dataBase->RequestToDB("", category);
    } else if (category == "Комедия") {
        query = "SELECT f.title AS \"Название фильма\", f.description AS \"Описание фильма\" FROM film f "
                "JOIN film_category fc ON f.film_id = fc.film_id "
                "JOIN category c ON c.category_id = fc.category_id "
                "WHERE c.name = 'Comedy'";
        dataBase->RequestToDB(query, category);
 ...
 * \param typeRequest
 */
void MainWindow::ScreenDataFromDB(QAbstractItemModel *model, QString typeRequest)
{

    ///Тут должен быть код ДЗ

    if (ui->tb_result->model()) {
        delete ui->tb_result->model();
    }
